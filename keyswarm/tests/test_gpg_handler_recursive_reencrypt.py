...
from keyswarm.gpg_handler import recursive_reencrypt, list_packages, get_binary
...
def test_gpg_handler_recursive_reencrypt_no_recursion():
    with TemporaryDirectory() as tmp_gpg_home:
        ...
        owner_trust1_cmd = 'echo "80035649BDABA4EC6A02E7D36BF58E6E9B697F1C:6:" | %s --homedir "{}" --import-ownertrust' % (get_binary(),)
        system(owner_trust1_cmd.format(tmp_gpg_home))
        owner_trust2_cmd = 'echo "A76F69733F4FBE97A4553B7C414129BA435C3226:6:" | %s --homedir "{}" --import-ownertrust' % (get_binary(),)
        ...
        with TemporaryDirectory() as tmp_password_store:
            ...
            print(packages)
            assert 'ID 304C48B44BD634E0' in packages and 'ID BD0E6C2E8C5A523C' in packages and len(packages) == 2
